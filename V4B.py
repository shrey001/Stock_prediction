...
future_days = 126      # 6 months
top_n = 10
trading_cost = 0.002   # 0.20% cost per 6-month trade
...
rebalance_dates = all_dates.groupby(
    [all_dates.dt.year, all_dates.dt.month]
).max().tolist()

results = []

print("\nRunning honest 6-month backtest...")
...
for test_date in rebalance_dates:
    ...
    test_data["Prediction"] = model.predict_proba(X_today)[:, 1]
...
if results_df.empty:
    print("No backtest results. Try using an earlier start_date.")
    quit()

# Since returns overlap monthly, use average 6-month return instead of compounding every month
...
print(f"Win rate vs SPY: {portfolio_win_rate:.2%}")
print(f"Estimated annualized portfolio return: {annualized_portfolio:.2%}")
print(f"Estimated annualized SPY return: {annualized_spy:.2%}")

print("\nMost recent top picks:")
print(results_df.tail(1)["Top_Picks"].values[0])

results_df.to_csv("honest_backtest_results.csv", index=False)

print("\nSaved results to honest_backtest_results.csv")